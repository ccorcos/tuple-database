...
This file is generated from async/ReactiveAsyncTupleDatabase.test.ts
...
import { ReactiveTupleDatabaseClient } from "./ReactiveTupleDatabaseClient"
import { ReactiveTupleDatabaseApi } from "./types"

function reactiveStorageTestSuite(
	name: string,
	createStorage: () => ReactiveTupleDatabaseApi
) {
	describe(name, () => {
		it("works with set", () => {
...
reactiveStorageTestSuite(
	"ReactiveTupleDatabaseClient(ReactiveTupleDatabase(InMemoryTupleStorage))",
	() =>
		new ReactiveTupleDatabaseClient(
			new ReactiveTupleDatabase(new InMemoryTupleStorage())
		)
)
