...
import { ReactiveAsyncTupleDatabaseClient } from "./ReactiveAsyncTupleDatabaseClient"
import { ReactiveAsyncTupleDatabaseApi } from "./types"

function reactiveStorageTestSuite(
	name: string,
	createStorage: () => ReactiveAsyncTupleDatabaseApi
) {
	describe(name, () => {
...
reactiveStorageTestSuite(
	"ReactiveAsyncTupleDatabase(InMemoryTupleStorage)",
	() => new ReactiveAsyncTupleDatabase(new InMemoryTupleStorage())
)

reactiveStorageTestSuite(
	"ReactiveAsyncTupleDatabaseClient(ReactiveAsyncTupleDatabase(InMemoryTupleStorage))",
	() =>
		new ReactiveAsyncTupleDatabaseClient(
			new ReactiveAsyncTupleDatabase(new InMemoryTupleStorage())
		)
)
