...
import { transactionalQueryAsync } from "./transactionalQueryAsync"
	...
	describe(name, () => {
		...
		describe("MVCC - Multi-version Concurrency Control", () => {
			...
			if (!isSync) {
				it("transactionalAsyncQuery will retry on those errors", async () => {
					...
					const incScore = transactionalQueryAsync<Schema>()(
			...
			it("should probably generalize to scans as well", async () => {
				...
				const addScore = transactionalQueryAsync<Schema>()(
		...
		describe("subscribeQueryAsync", () => {
			...
			it("can transactionally read", async () => {
				...
				const getTotal = transactionalQueryAsync<Schema>()(async (tx) => {
		...
		describe("subspace", () => {
			it("get/exists/scan works", async () => {
				...
				const writePerson = transactionalQueryAsync<Schema>()(
